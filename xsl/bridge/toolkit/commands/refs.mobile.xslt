...
<xsl:stylesheet version="1.0"
            ...
            <a>
                ...
                <xsl:attribute name="href">
                    javascript:parent.frames[0].goToLink('<xsl:value-of
                        select="document($urn_resource_file)//target[parent::urn[contains(@name, $reqdm)]]"/>', '
                    <xsl:number format="1" value="count(preceding::xref|preceding::refdm|preceding::reqdm) + 1"/>')
                </xsl:attribute>
    ...
    <xsl:template match="refdm">
        ...
        <xsl:choose>
            ...
            <xsl:when test="parent::pmentry">
                <a target="content">
                    <xsl:attribute name="href">javascript:parent.frames[0].goToLink('<xsl:value-of
                            select="$dmFileName"/>', '0')<!--#<xsl:value-of select="$pmTitle"/-->
                    </xsl:attribute>
                    <xsl:value-of select="$linkName"/>
                </a>
            </xsl:when>
            ...
            <xsl:otherwise>
                <a>
                    ...
                    <xsl:attribute name="href">
                        javascript:parent.frames[0].goToLink('<xsl:value-of select="$dmFileName"/>', '<xsl:number
                            format="1" value="count(preceding::xref|preceding::refdm|preceding::reqdm) + 1"/>');
                    </xsl:attribute>
                    <xsl:value-of select="$linkName"/>
                </a>
            </xsl:otherwise>
    ...
    <xsl:template match="xref">
        ...
        <a>
            ...
            <xsl:attribute name="href">javascript:parent.frames[0].goToLink('<xsl:value-of select="@xlink:href"/>', '
                <xsl:number format="1" value="count(preceding::xref|preceding::refdm|preceding::reqdm) + 1"/>');
            </xsl:attribute>
    ...
    <xsl:template name="dmref" match="dmRef">
        ...
        <xsl:variable name="xlinkhref">
            <xsl:value-of
                    select="document('../../../../ViewerApplication/app/urn_resource_map.xml')//target[parent::urn[@name=$xlinkurn]]"/>
        </xsl:variable>
      ...
      $refinfocode,$refinfocodevariant,'-',$refitemlocationcode,'-',$reflearncode,$reflearneventcode)"/>
        ...
        <xsl:variable name="ref_dmcFile">
            <xsl:value-of
                    select="document('../../../../ViewerApplication/app/urn_resource_map.xml')//target[parent::urn[@name=$urn_string]]"/>
        </xsl:variable>
